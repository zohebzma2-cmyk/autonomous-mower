...
| Sweep angle | rows run along the direction (east-west or any boundary edge) with the fewest rows; rows spread evenly edge to edge; every leg routed | a 10 × 60 m strip got 51 turns, the far-edge sliver measured 93 %, and an L yard's inside corner got cut — all found by sweeping 8 yard shapes through both planners with the measured-coverage check |
